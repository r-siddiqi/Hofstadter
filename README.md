...
```math
$$H = \omega_0\underbrace{\sum_i a_i^{\dagger} a_i}_\textrm{all latice sites} \underbrace{\sum_{-t \langle i,j\rangle} (a_i^{\dagger} a_j + a_j^{\dagger} a_i)}_\textrm{hopping between nearest neighbors}$$
```
